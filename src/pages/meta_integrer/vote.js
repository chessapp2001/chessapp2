...
class Vote extends React.Component {
  ...
  no = async () => {
    await oxygenIntegrity.methods.VoteNo().call();
    await oxygenIntegrity.methods.oxygens().call();
  ...
  render() {
    return (
      <div className="glosowanie">
        <button className="positive ui button" onClick={this.leszy}>
          Wyświetl adres Leszego, na który musisz oddać głos
        </button>

        <h3> {this.state.Leszy}</h3>

        <button className="positive ui button" onClick={this.yes}>
          TAK
        </button>
        <h3> {this.state.Feedback}</h3>

        <button clclassName=egative ui button" onClick={this.no}>
          NIE
        </button>
