...
class MyComponent extends React.Component {
  ...
  addOffer = async () => {
   ...
   await Promise.all(
    data.map(async (item) => {
      const result = await oxygenIntegrity.methods.remember(accounts[0]).call();
      if (result === item) {
        deployed = item;
    ...
    await oxygen.methods.addOffer( area,web3.utils.toWei(price,'ether'), lease_time, owner).send({ gas: 20000000, from: accounts[0]});
    this.setState({ accounts: true});
    }
    else
    await oxygen.methods.addOffer( area, price, lease_time, owner).send({ gas: 20000000, from: accounts[0]});
  ...
  render() {
    ...
    return (
      <div>
       ...
       <div class="dupa"> <h1 >Wielkość działki:</h1></div>
        <div class="ui  input">
        <input
          type="text"
          name="area"
        ...
        <div class="ui green inverted segment dipa">
        <h1>Cena:</h1> <div class="ui input">
        <input
          type="text"
          name="price"
          value={this.state.price}
          onChange={this.handlePriceChange}
        /></div> <div class="ui basic label">
        eth
      </div></div>
        <div class="ui green inverted segment dipa">
        <h1>Termin wynajmu: </h1> <div class="ui input">
        <input
          type="text"
          name="lease_time"
        ...
        <div class="ui green inverted segment dipa">
        <h1>Właściciel:</h1> <div class="ui input">
        <input
          type="text"
          name="owner"
        ...
        <button class="button button1" onClick={this.addOffer.bind(this, App)}>Stwórz kontrakt</button>
